...
int client_handle_connection(int client_socket_fd, NINEDOF *ninedof)
{
	...
	memset(buffer, 0, 256);
	sprintf(buffer, "time (epoch), pitch, roll, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, mag_x, mag_y, mag_z, temperature");
    ...
    double ax=ninedof->accel_data.x;
    double ay=ninedof->accel_data.y;
    double az=ninedof->accel_data.z;
    double pitch = -atan2(ax,sqrt(ax*ax+ay*ay+az*az))*180/PI;
    double roll = atan2(ay,sqrt(ax*ax+ay*ay+az*az))*180/PI;
    
		// write 9DOF reading to buffer
		sprintf(buffer, "%10.10f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f", 
			sec_since_epoch, 
			pitch, 
      roll, 
      ninedof->accel_data.x;
      ninedof->accel_data.y;
