...
data_t read_bias(mraa_i2c_context accel)
{
	int16_t raw_accel_x, raw_accel_y, raw_accel_z;
	uint8_t data_byte[6];
	data_t data;
...
data_t read_accel_calibrated(mraa_i2c_context accel, float a_res, data_t accel_bias)
{
	int16_t raw_accel_x, raw_accel_y, raw_accel_z;
	uint8_t data_byte[6];
	data_t data;
	...
	//printf("\t%d \t%d \t%d\n", raw_accel_x, raw_accel_y, raw_accel_z);
	data.x = (raw_accel_x - accel_bias.x) * a_res;
	data.y = (raw_accel_y - accel_bias.y) * a_res;
	data.z = (raw_accel_z - accel_bias.z ) * a_res + 1.0;
...
data_t read_gyro_bias(mraa_i2c_context gyro)
{
	int16_t raw_gyro_x, raw_gyro_y, raw_gyro_z;
	uint8_t data_byte[6];
	data_t data;
...
data_t read_gyro_calibrated(mraa_i2c_context gyro, float g_res, data_t gyro_bias)
{
	int16_t raw_gyro_x, raw_gyro_y, raw_gyro_z;
	uint8_t data_byte[6];
	data_t data;
	...
	//printf("\t%d \t%d \t%d\n", raw_gyro_x, raw_gyro_y, raw_gyro_z);
	data.x = (raw_gyro_x - gyro_bias.x) * g_res;
	data.y = (raw_gyro_y - gyro_bias.y) * g_res;
	data.z = (raw_gyro_z - gyro_bias.z) * g_res;
